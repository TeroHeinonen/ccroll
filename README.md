...
```
ccroll 0.1.0  ·  auto-rotate at session≥95% / fable≥97% or ≤60s from a limit · early to next reset when runway>3h  ·  peak hold 05:00–11:00 PDT  ·  22:04:31
...
`--peak-hold HH:MM-HH:MM` sets the range (a range past midnight is fine), `--peak-tz` the IANA zone its clock is read in — the range is resolved on that wall clock, so it stays at 05:00 local across a DST change — and `--no-peak-hold` turns it off.

Tuning: `--threshold`, `--scoped-threshold`, `--lead` (seconds of burn-predicted headroom at which to rotate early, default 60 or one poll interval if longer), `--interval` (active-account poll, default 60 s), `--scan` (full-fleet scan, default 300 s), `--preempt-runway` (hours, default 3), `--no-preempt`, `--touch`, `--grace` (post-swap seconds without burn-based rotation or pre-emption, default 300), `--preempt-max-cost` (percent of the governing window a swap may cost before pre-emption is skipped, default 5), `--cooldown` (default 0), `--no-rotate` (observe only), `--peak-hold`, `--peak-tz`, `--no-peak-hold` (see **Peak-hour hold**), `--sync-identity` (off by default, see **Caveats**), `--signal-dir`, `--no-signal` (see **Account-switch signals**).
