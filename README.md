...
**Static thresholds are the latest point, not the trigger.** With a fan-out of subagents an account can burn 150%/h of its session window: 95% → 100% takes two minutes, which is about one poll plus one swap — too late, and every request in flight dies with a 429. So ccroll predicts the time to each limit from the burn estimate and rotates once that prediction drops under the lead time, however far from the static threshold the account still is. The estimate is deliberately pessimistic: the larger of the 45-minute least-squares slope and the recent step slope, so a burst that started two minutes ago is caught before the fit catches up. It restarts at every swap (samples taken while the account sat idle in the fleet scan say nothing about how it burns once live), a spike must persist across two consecutive polls before it counts (the first poll after a swap shows every running agent re-priming its context at once — a one-time jump, not a rate), and early rotation only applies from 50% of a window upward; below that the static thresholds alone decide. The active account is never polled faster than `--interval`, however close a limit is. An earlier version dropped to a 15-second poll under ten minutes from a limit; every account that entered it was throttled by the usage endpoint in those very minutes, which looked like the poll's doing — it was not (see below), but a faster poll buys nothing either. What a close limit needs is the projection (the last reading advanced by rate × age) and the lead, not more reads. No burn estimate (the post-swap grace plus about two minutes) means the static thresholds alone apply.

**Every reading is a lower bound, and is acted on as one.** The agents keep consuming between polls, and for as long as the endpoint refuses to answer, so the figure ccroll last read is never where the account is now. Each window of the active account is therefore projected forward by its measured burn times the age of the reading before any threshold or lead is compared against it, and that comparison runs every second rather than at the next poll — the poll cadence corrects the model, it is not the trigger. This matters most in a blackout: a snapshot frozen at 91% can never cross a 95% threshold however long the endpoint stays silent, which is how one account was held at "91%" for eleven minutes while it burned through to 100%. Projected, that same reading crosses 95% two minutes into the blackout and rotation fires with the reason spelling out its working (`session ≈95% (91% read 2m ago + 111%/h)`). The projection uses the burn that was measured before the reading went stale; a burst that begins during a blackout is still invisible, as it would be to any observer without data.
...
**Which weekly limit governs is a flag.** By default (`--by weekly`) ccroll governs by the all-models weekly limit: it is the constraint when you work on a model without its own weekly cap (e.g. Opus). Plans with a per-model weekly limit (e.g. the Fable limit on current Max plans) hit *that* wall first when you work on that model; use `--by scoped` to treat it as the constraint and pick the next account by that window.
...
**When the weekly window is the bottleneck, ccroll also moves early.** If the fleet is Fable-bound rather than session-bound, the account to *sit on* is the one whose Fable window resets next: its unused headroom is the first to vanish, and being on it at the moment it resets reopens its next week with no idle gap. So while the active account still has headroom, ccroll moves to the account with the earliest reset among the comfortable candidates. The move is gated three ways: the active account's predicted time to any limit must exceed `--preempt-runway` (default 3 h), which keeps pre-emption off entirely while sessions are the constraint (there a pre-emptive swap would only add a cache re-prime); the measured swap cost must be within `--preempt-max-cost`; and the lead has to be worth the swap. A lead of *L* hours on the reset can rescue at most burn × *L* percent of headroom — the two accounts differ only in the order they are drained, and that order can only matter during the interval where one window is still open and the other has already reset — while the swap costs the measured preload on that window. ccroll moves only when burn × *L* exceeds the preload by at least the 1 % step the endpoint reports usage in (a preload measured as 0 % is below 1 %, not free). Both sides are measured, so the bar scales with the burn: at 5 %/h a lead pays from about twelve minutes, at 0.5 %/h from two hours, and a one-minute lead never does. There is deliberately no minimum interval between moves — see below. In simulation of a Fable-bound fleet at weekly capacity this cut blocked time roughly 2–3× (steady 3.6% → 1.2%, bursty 4.5% → 0.4%) for about one extra swap a day. `--no-preempt` restores rotate-only-when-spent. `--touch` goes one step further and also opens freshly reset windows at once (one request there, then on to the next-reset account); that adds another ~1.5 swaps a day and only pays when a swap costs no more than about 1% of the weekly window, so it is opt-in. Touching is the one move that can chain — opening a window removes it from the fresh set and the next fresh account inherits the target — so it is held to **one touch per natural cycle**: after a touch, no further touch until an exhaustion-driven swap has happened. That guard is expressed in the fleet's own rhythm rather than in minutes, so it holds whether a cycle takes ten minutes or ten hours.

**There is no swap cooldown by default**, and for ordinary rotation none is needed. Rotation proper requires the account you are leaving to be spent and the one you are moving to not to be; usage within a window only rises, and only the active account consumes. Returning to an earlier account therefore requires that account's window to have reset, which is recovery rather than flapping. Pre-emption is the one path that leaves a healthy account, and it needs no floor either: it only ever moves to an account whose reset is *strictly earlier* than the current one's by a lead worth the swap, so the account just left can never be chosen again — targets descend in reset time and run out. The worth gate is what makes that ordering reliable: the usage endpoint computes every `resets_at` relative to the read and rounds it to the minute, so two windows that truly reset within the same minute read as "one minute sooner" in either direction depending on the second they were polled at. A plain strictly-earlier comparison ping-ponged between such a pair every few minutes, paying a preload each way; a lead worth a swap is many minutes at any burn the runway gate admits, far beyond the rounding. An earlier version carried a fixed 15-minute floor between moves as a precaution; simulation across cycle lengths from ten minutes to ten hours showed it byte-identical to no floor under the gates above, and useless against the one real chaining risk (`--touch`), which is now guarded by the per-cycle rule instead. Set `--cooldown` if you want a global minimum anyway.

**When every account is spent**, ccroll doesn't give up: it waits for the first moment any account frees up (the latest reset among that account's binding limits), then rescans and rotates as soon as headroom exists. The banner names the account that rescan will actually land on, judged by the same rules at that moment, not merely the one whose reset ticks first: several windows often reset within the same minute, and when a spent session and a fresh weekly window free up together the fresh weekly one is taken. Only resets still ahead count, and no rescan is ever scheduled less than 30 seconds out — a reset time already in the past names a moment that has happened, and treating it as one to wait for turned the watch loop into a scan of every account every second, which is itself enough to get the usage endpoint to throttle you. The banner clears as soon as a fallback has headroom again, rather than lingering as a description of a state that has passed.
...
The hold is a state, not a single act. Its desired condition is *the active account refuses requests*, and ccroll keeps that true for the length of the range: if the parked account's blocking window is about to reset — `--lead` seconds before it, the same margin early rotation uses — it re-parks on another refused account, so a five-hour session limit cannot let the fleet slip back to work at 08:00. Among several refused accounts it takes the one refused longest, which needs the fewest re-parks; it is confirmed by a fresh read before the swap, since an account that reset since the last scan would hold nobody. When *no* account is refused there is nothing to park on: ccroll says so on the dashboard and the active account keeps working until one appears — quite possibly the active account itself, once it hits its own limit, in which case ccroll simply stays. Missing data never parks, as it never rotates. Burn-based early rotation and pre-emption are off during the hold, because the whole point is to be on a spent account, and the `switch_expected` it would otherwise keep predicting for an account at 100 % is silenced with them.

`r` ends the hold for that day's range: the rotation it triggers is the normal one, no re-park follows even though the range is still running, and the next hold is tomorrow's. The release is remembered, so restarting ccroll inside the range does not re-park; restarting inside the range *without* an earlier `r` does, since the range is then simply in force. `p` (pause) covers the hold as it covers every automatic swap. A park is not a natural end of a cycle, so it does not re-arm `--touch`, and it contributes no preload measurement: nothing re-primes on an account that refuses it, so the measured cost would be a false 0 %. The resume does measure, like any swap onto a working account.
...
| Event | When | Fields |
|---|---|---|
| `switch_expected` | once, about 3 minutes ahead of a predicted or scheduled swap; again only if the ETA moves by more than 3 minutes | `eta_utc`, `usage_pct` |
